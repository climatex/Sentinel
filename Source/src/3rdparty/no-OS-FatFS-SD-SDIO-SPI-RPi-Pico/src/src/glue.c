...
// FATFS reused for SD card and hard drive access
// project usage: dos.cpp, sd.cpp
bool diskio_use_sd = true; // true: SD, false: hard drive
extern DRESULT dosDiskRead(BYTE pdrv, BYTE *buff, LBA_t sector, UINT count);
extern DRESULT dosDiskWrite(BYTE pdrv, const BYTE *buff, LBA_t sector, UINT count);
extern DRESULT dosIoctl(BYTE pdrv, BYTE cmd, BYTE *buff);
...
DRESULT disk_read(BYTE pdrv,  /* Physical drive number to identify the drive */
    ...
    {
        return dosDiskRead(pdrv, buff, sector, count);
    }
}
...
DRESULT disk_write(BYTE pdrv, /* Physical drive number to identify the drive */
    ...
    {
        return dosDiskWrite(pdrv, buff, sector, count);
    }
